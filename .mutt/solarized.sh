...
J_red           color160       red
J_magenta       color127       magenta
J_violet        color61        brightmagenta
J_blue          color33        blue
J_cyan          color37        cyan
J_green         color64        green
J_bg            color234       default
J_fg            color244       brightyellow
J_statusfg      color241       black
J_statusbg      color235       default
SETPALETTEDARK

cat > light.tmp <<SETPALETTELIGHT
...
cat > palette.awk <<'PALETTEAWK'
NR == FNR { a[$1] = $col; next }
          { for (i in a) gsub(i " +", sprintf("%-18s", a[i])); print }
PALETTEAWK

rm "solarized-dark-16.muttrc" 2>/dev/null
rm "solarized-dark-256.muttrc" 2>/dev/null
rm "solarized-light-16.muttrc" 2>/dev/null
rm "solarized-light-256.muttrc" 2>/dev/null

SOURCE="solarized.muttrc.template"

awk -f palette.awk col=3 dark.tmp $SOURCE  > solarized-dark-16.muttrc
awk -f palette.awk col=2 dark.tmp $SOURCE  > solarized-dark-256.muttrc
awk -f palette.awk col=3 light.tmp $SOURCE > solarized-light-16.muttrc
awk -f palette.awk col=2 light.tmp $SOURCE > solarized-light-256.muttrc
